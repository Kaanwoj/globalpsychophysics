...
// The variances are estimated for each matching direction and production factor
...
data {
  ...
  array[ntotal_lb] int thisP_lb;    // production factor for each trial
  array[ntotal_bl] int thisP_bl;
}
transformed data {
...
parameters {
  ...
  real<lower=0> omega1;
  real<lower=0> omega;
  vector<lower=0>[np] sig_lb;
  vector<lower=0>[np] sig_bl;
...
generated quantities {
  array[ntotal] real tgt_pred = normal_rng(
    ...
    append_row(sig_lb[thisP_lb], sig_bl[thisP_bl]));
}
