  ...
  real loud_to_bright(real std_loud, real alpha_l, real alpha_b, real beta_l,
    ...
    return db_lambert(
      ...
      pow(weigh_fun(p, omega1, omega) * alpha_l * pow(db_inv_spl(std_loud), beta_l) -
            weigh_fun(p, omega1, omega) * alpha_l * pow(db_inv_spl(rho_l), beta_l) +
    ...
    return db_spl(
       inv(alpha_l) *
         pow(weigh_fun(p, omega1, omega) * alpha_b * pow(db_inv_lambert(std_bright), beta_b) -
               weigh_fun(p, omega1, omega) * alpha_b * pow(db_inv_lambert(rho_b), beta_b) +
               alpha_l * pow(db_inv_lambert(rho_l), beta_l),
             inv(beta_l))
    );
  }
}
