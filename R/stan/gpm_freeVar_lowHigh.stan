...
// Two variances are estimated for each matching direction: one for low 
//      standard intensities and one for middle to high standard intensities
...
data {
  ...
  int<lower=1> np;
  int<lower=1> nvar_task;   // total nvar per task
  array[np] int p;
  int<lower=1> nstd_lb;
  int<lower=1> nstd_bl;
  ...
  array[ntotal_lb] int low_high_std_lb;    // marks low and high standards
  array[ntotal_bl] int low_high_std_bl;
}
transformed data {
...
parameters {
  ...
  real<lower=0> omega1;
  real<lower=0> omega;
  vector<lower=0>[nvar_task] sig_lb;
  vector<lower=0>[nvar_task] sig_bl;
...
model {
  ...
  tgt ~ normal(append_row(to_vector(mu_lb)[std_p_lb],
               ...
               append_row(sig_lb[low_high_std_lb], 
                          sig_bl[low_high_std_bl]));
